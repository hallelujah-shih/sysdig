...
bool sinsp_parser::reset(sinsp_evt *evt)
{
	...
	{
		auto container_id = sinsp_container_info::get_id_from_json(evt);
		evt->m_tinfo = m_inspector->m_container_manager.get_container_tinfo(container_id).get();
	}
	else
	{
...
{
	sinsp_container_info container_info = sinsp_container_info::from_container_json(evt);
