...
void docker_async_source::set_top_tid(const std::string &container_id, sinsp_threadinfo *tinfo)
{
	...
	g_logger.format(sinsp_logger::SEV_DEBUG,
			"docker_async (%s): Setting top tid %ld",
			container_id.c_str(),
			tinfo->m_tid);
...
int64_t docker_async_source::get_top_tid(const std::string &container_id)
{
	...
	{
		g_logger.format(sinsp_logger::SEV_DEBUG,
				"docker_async (%s): Getting top tid %ld",
				container_id.c_str(),
				cacc->second);
		return cacc->second;
	}
	...
	g_logger.format(sinsp_logger::SEV_ERROR,
			"docker_async (%s): Did not find container id->tid mapping for in-progress container metadata lookup?",
			container_id.c_str());
	return 0;
}
...
bool docker_async_source::pending_lookup(std::string &container_id)
{
	...
	bool res = m_top_tids.find(cacc, container_id);

	g_logger.format(sinsp_logger::SEV_DEBUG,
			"docker_async (%s): pending_lookup returning %s",
			container_id.c_str(),
			(res ? "true" : "false"));
...
void docker_async_source::update_top_tid(std::string &container_id, sinsp_threadinfo *tinfo)
{
	   ...
	   top_tinfo->m_flags & PPM_CL_CLOSED)
	{
		...
		// the same container.
		g_logger.format(sinsp_logger::SEV_DEBUG,
				"docker_async (%s): top tid %ld exited, finding new top tid",
				container_id.c_str(),
				top_tid);
		...
		g_logger.format(sinsp_logger::SEV_DEBUG,
				"docker_async (%s): Found new top tid %ld",
				container_id.c_str(),
				top_tinfo->m_tid);
...
bool docker::resolve(sinsp_container_manager* manager, sinsp_threadinfo* tinfo, bool query_os_for_missing_info)
{
	    ...
	    query_os_for_missing_info)
	{
		...
		{
			g_logger.format(sinsp_logger::SEV_DEBUG,
					"docker_async (%s): Scheduling container info lookup",
					container_id.c_str());
		...
		{
			g_logger.format(sinsp_logger::SEV_DEBUG,
					"docker_async (%s): Lookup in progress, updating top tid",
					container_id.c_str());
